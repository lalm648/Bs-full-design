...
(function () {
  ...
  function initPLP() {
    ...
    function renderGrid() {
      ...
      if (!list.length) {
        ...
        var editorialBreaks = [8, 16];
        var html = "";
        shown.forEach(function (p, i) {
          html += cardHTML(p);
          if (editorialBreaks.indexOf(i + 1) !== -1 && i !== shown.length - 1) {
            html += editorialHTML(EDITORIALS[editorialBreaks.indexOf(i + 1) % EDITORIALS.length]);
          }
        });
    ...
    plpGrid.addEventListener("click", function (e) {
      ...
      if (sizeBtn) {
        e.preventDefault();
        e.stopPropagation();
        var card = sizeBtn.closest(".bs-card");
        card.querySelectorAll("[data-card-size]").forEach(function (btn) {
      ...
      if (quickBtn) {
        ...
        var quickCard = quickBtn.closest(".bs-card");
        var selected = quickCard.querySelector("[data-card-size].is-selected") || quickCard.querySelector("[data-card-size]");
        var sizeValue = selected ? selected.dataset.cardSize : "OS";
        if (selected) {
          quickCard.querySelectorAll("[data-card-size]").forEach(function (btn) {
  ...
  function initPDPStatic() {
    ...
    var swatch = pdpField("colorSwatch");
    if (colorGroup) colorGroup.hidden = !p.colorHex;
    if (swatch && p.colorHex) {
      swatch.style.background = p.colorHex;
      swatch.setAttribute("title", p.color);
    }
    ...
    if (relatedGrid) {
      ...
      pool.slice(0, 4).forEach(function (item) {
        var card = cloneTemplate("pdpRelatedCardTemplate");
        var wished = wishlist.indexOf(item.id) !== -1;
        var cardSale = item.compareAt != null && item.compareAt > item.price;
        var link = card.querySelector(".bs-card-link");
        var quickLink = card.querySelector("[data-related-link]");
        var mainImg = card.querySelector(".bs-card-img-main");
        var hoverImg = card.querySelector(".bs-card-img-hover");
        var badgeWrap = card.querySelector(".bs-card-badge");
        var badge = card.querySelector(".bs-badge");
        var wish = card.querySelector(".bs-card-wish");
        var add = card.querySelector("[data-card-add]");
        var priceWrap = card.querySelector('[data-card="priceWrap"]');
        var compare = card.querySelector('[data-card="compare"]');

        card.dataset.id = item.id;
        card.classList.toggle("has-hover", !!item.hover);
        link.href = "product.html?id=" + item.id;
        link.setAttribute("aria-label", item.name);
        if (quickLink) {
          quickLink.href = "product.html?id=" + item.id;
          quickLink.setAttribute("aria-label", "Quick view " + item.name);
        }
        mainImg.src = item.image;
        mainImg.alt = item.name;
        if (item.hover) {
          hoverImg.hidden = false;
          hoverImg.src = item.hover;
        }
        if (item.soldOut || item.badge) {
          badgeWrap.hidden = false;
          badge.textContent = item.soldOut ? "Sold out" : item.badge;
          badge.classList.add("bs-badge-" + (item.soldOut ? "soldout" : (item.badgeTone || "neutral")));
        }
        wish.dataset.wish = item.id;
        wish.classList.toggle("is-wishlisted", wished);
        wish.setAttribute("aria-label", wished ? "Remove from wishlist" : "Add to wishlist");
        if (add) {
          add.disabled = !!item.soldOut;
          add.textContent = item.soldOut ? "Sold out" : "Add to cart";
        }
        card.querySelector('[data-card="vendor"]').textContent = item.vendor;
        card.querySelector('[data-card="name"]').textContent = item.name;
        card.querySelector('[data-card="price"]').textContent = money(item.price);
        priceWrap.classList.toggle("on-sale", cardSale);
        if (cardSale) {
          compare.hidden = false;
          compare.textContent = money(item.compareAt);
        }
        relatedGrid.appendChild(card);
      });
      bindWishClicks(relatedGrid, null);
      relatedGrid.addEventListener("click", function (e) {
        var addBtn = e.target.closest("[data-card-add]");
        if (!addBtn) return;
        var card = addBtn.closest(".bs-card");
        var item = products.filter(function (x) { return x.id === card.dataset.id; })[0];
        if (!item || item.soldOut) return;
        var size = item.sizes.filter(function (s) { return s !== "OS"; })[0] || "OS";
        addLine(item.id, size, 1);
        toast("Added to bag - " + item.vendor);
      });
    }
    ...
    if (relatedEl) {
      ...
      relatedEl.addEventListener("click", function (e) {
        ...
        if (quickBtn) {
          e.preventDefault();
          e.stopPropagation();
          var product = prod(quickBtn.dataset.cardAdd);
          if (!product || product.soldOut) return;
          var selected = quickBtn.closest(".bs-card").querySelector("[data-card-size].is-selected") ||
                         quickBtn.closest(".bs-card").querySelector("[data-card-size]");
          if (!selected) return;
          addLine(product.id, selected.dataset.cardSize, 1);
          render();
          toast("Added to bag - " + product.vendor + ", size " + selected.dataset.cardSize);
        }
      });
    }
